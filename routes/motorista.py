...
async def obter_rota_atual(current_user_id: str):
    """
    ...
    return {**rota_data, **direcionamento_rota}
...
async def iniciar_rota(rota_id: str):
    ...
    rota_ref.update({"inicio": datetime.now().isoformat(), "status": True})
...
async def finalizar_rota(rota_id: str):
    ...
    rota_ref.update({"fim": datetime.now().isoformat(), "status": False})
